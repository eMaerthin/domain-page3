...
function outcomeChart(treatment: any, rows: any[], videosById: Map<string, any>, locale: Locale): string {
  ...
  const box = (rowsForGroup: any[], x: number, color: string, label: string, group: "treated" | "control") => {
    ...
    const points = rowsForGroup.map((row, index) => `<circle class="causal-dot" data-group="${group}" data-video-id="${escapeHtml(row.video_id || "")}" cx="${x + ((index * 37) % 34) - 17}" cy="${y(row[outcomeKey])}" r="3.5" fill="${color}" fill-opacity=".48"><title>${escapeHtml(row.title || row.video_id || "")} · ${label}: ${Number(row[outcomeKey]).toFixed(3)}</title></circle>`).join("");
...
function renderCausalPanel(causal: any, rows: any[], videosById: Map<string, any>): void {
  ...
  target.querySelectorAll<SVGCircleElement>(".causal-dot").forEach((dot) => {
    ...
    dot.addEventListener("click", () => {
      ...
      detail.classList.remove("group-treated", "group-control");
      detail.classList.add(dot.dataset.group === "control" ? "group-control" : "group-treated");
    ...
    <main>
      ...
      <section class="section feature-diff-section">
        ...
        <div class="feature-diff-list">${featureDifferences.map((item: any) => {
          ...
          const maxValue = Math.max(Math.abs(topValue), Math.abs(worstValue), .001);
